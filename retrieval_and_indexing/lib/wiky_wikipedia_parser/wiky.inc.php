...
class wiky {
	...
	public function __construct($analyze=false) {
		$this->patterns=array(
			// Links
			"/\[\[(?!File)(?!Image)[^\|^\]]+?\|(.+?)\]\]/",		// Link in the form of [[wikilink|displayname]]
			"/\[\[(?!File)(?!Image)(.+?)\]\]/",					// Simple link
			"/\[\[Image(.+?)\]\]/",				// Image
			"/\[\[File(.+?)\]\]/",				// File
			"/<ref[^>]*?\/>/",					// Reference / Citation
			"/<ref[^>]*?>[\s\S]+?<\/ref>/",		// Reference / Citation
			...
			// Cleanup	
			"/\(\s*?\)/",			// Empty brackets (might be the result of some transformations done earlier)
			"/\(\s*?[;,]/",
			"/\s*?[;,]\s*?\)/",
		);
