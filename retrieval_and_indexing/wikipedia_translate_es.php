...
$filename = "es_de_translated_relevant_articles_credibility.txt";
$translations = array();
$count = 0;
    ...
    while (($row = fgetcsv($handle, 1000, ';')) !== FALSE) {
        ...
        if ($row[3] == "") {
            if ($yandex_limit_reached) {
                ...
                $translation = @file_get_contents("https://translate.yandex.net/api/v1.5/tr.json/translate?key=" . YANDEX_KEY . "&lang=en-es&text=" . urlencode($row[0]));
                if ($translation) {
                    $translation = json_decode($translation, true);
                    $translation = $translation["text"][0];
                    //yandex translation
                    array_push($translations, $row[0] . ";" . $row[1] . ";" . $row[2] . ";" . $translation);
            ...
            array_push($translations, $row[0] . ";" . $row[1] . ";" . $row[2] . ";" . $row[3]);
